...
class ListTodo {
    ...
    showAllTasks() {
        return this.list.map((e, i) => `<div class="single-task d-flex justify-content-between ${e.isDone? 'success':e.priority}-task border px-2 py-3 my-1">
        <div class="task-name">
        <input type="checkbox" class="task-status" name="done-task-${e.id}" id="done-${e.id}" class="">
       <span class="font-weight-bold">${++i}. ${e.name}</span>
        </div>
       <div>
       <button class="btn btn-success task-success" name="success-${e.id}" id="success-${e.id}">${e.isDone?'Undone':'Done'}</button>
